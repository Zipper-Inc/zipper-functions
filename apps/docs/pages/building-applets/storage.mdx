...
Zipper also has basic key value storage built-in. Each applet has its own unique storage instance, and it will not go with you when you fork an app. You can use it using the `[Zipper.storage](http://Zipper.storage)` global.
...
Get a storage key by value. You do not need to worry about unserializing.
