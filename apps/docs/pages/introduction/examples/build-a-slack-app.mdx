...
Click on "Event Subscriptions" in the left sidebar and enable events. You'll be asked to input a URL to send events to - you're going to enter your app's zipper.run URL (`[app-name].zipper.run`). You should see a green checkmark after a few seconds indicating that Slack was able to successfully send a request to your applet.
