...
**Bot Scopes**
These are the scopes for the API methods that you want to call on behalf of a bot user (i.e. you'll see data and take action from the perspective of the bot). Adding bot scopes will add a bot user to your Slack app automatically.
...
If your Zipper applet is going to be used by different users and you want them to be able to interact with the Slack API as themselve, you can set this field to true. This will add an `Authorize Slack` button above the inputs to your applet. When a user clicks this button, they'll be taken through the OAuth flow and we'll encrypt and store their API token for you.
