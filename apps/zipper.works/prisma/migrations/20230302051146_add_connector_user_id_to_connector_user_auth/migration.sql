/*
  ...
  - A unique constraint covering the columns `[appId,connectorType,connectorUserId]` on the table `app_connector_user_auth` will be added. If there are existing duplicate values, this will fail.
  ...
  - Added the required column `connectorUserId` to the `app_connector_user_auth` table without a default value. This is not possible if the table is not empty.
...
ADD COLUMN     "connectorUserId" TEXT NOT NULL,
ADD COLUMN     "encryptedAccessToken" TEXT NOT NULL,
ALTER COLUMN "metadata" SET NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX "app_connector_user_auth_appId_connectorType_connectorUserId_key" ON "app_connector_user_auth"("appId", "connectorType", "connectorUserId");
